...
that some measures fit naturally into a big data scenario.
...
or more commonly known as the average
\[
...
The `Beauty` dataset contains results for `463` classes. After loading 
the data set into R, can extract a particular column using the dollar 
operator
```{r, results="hide"}
...
ascending order. Ranked observations are denoted as $x_{(1)}, x_{(2)}, \ldots,
x_{(n)}$. The sample median is defined as:
\[
\text{Sample median} = 
\begin{cases}
  x_{(n+1)/2}, & n \text{ odd;} \\
...
R has built-in function called `median()` that we can use to calculate the median.
...
is symmetric. However the class size values differ: $\bar x = 55$ and the median is $29$. This
is because the class sizes are is [skewed](https://en.wikipedia.org/wiki/Skewness); 
...
As well as knowing the location statistics of a data set, we also need to know how variable 
or `spread-out' our data are.
...
smallest.^[When you get a new data set, calculating the range is
    useful when checking for obvious data-inputting errors.]
\[
\text{Range }= x_{(n)} - x_{(1)} 
\]
...
This is mainly because they have nice mathematical properties, unlike the median. 
...
deviations from the sample mean rather than the ``true" mean of the population
  we are sampling from] However, since $\sum x^2$ becomes large, in practice we would 
use
\[
...
deviation is preferred as a summary measure as it is in the units of the
...
R has builtin functions to calculate the standard deviation and variance
```{r}
...
is computationally more difficult to calculate.

### Numerical examples
  
For the following data sets, calculate the inter-quartile range:

1. $\{5, 6, 7, 8\}$ The lower quartile is the $5/4 = 1.25^{\text{th}}$ smallest
  observation, i.e. $x_{(1.25)}$. The value of $x_{(1)} = 5$ and $x_{(2)} = 6$. So
  \[
...
2. $\{10, 15, 20, 25, 50\}. The lower quartile is the $(5+1)/4 = 1.5^{\text{th}}$ smallest
...
both statistical and computationally. Even though we've only covered a few statistics,
some of them can be easily adapted to deal with streaming data.
...
\]
Again this is algorithm is [numerically stable](https://en.wikipedia.org/wiki/Numerical_stability).
...
```{r, 2-1, echo=FALSE, fig.cap = "The estimate of the mean age as we progress through the dataset."}
