...
that doesn't require any fancy statistics.
...
John Snow^[Not [that](https://en.wikipedia.org/wiki/Jon_Snow_(character)) Jon Snow] 
created a simple dot map to illustrate the cluster of cholera cases around the pump - figure \@ref(fig:3-1).
...
start generating plots to get a feel for the data set. It helps us detect interesting
...
The `stat_smooth()` layer adds a local trend linear. Essentially, the line moves 
...
Another interesting feature is the long tail - this is an example of a _right skewed_ data set.
...
By splitting age by gender, two interesting features become obvious from figure \@ref(fig:3-9)
...
could be by pure chance, user lying about their age, or a mistake in data entry.
...
The tricky mathematical part deals with what kernel should we use and how wide should that
kernel be. Intuitively, when we have many points, the kernel should be narrower. 
