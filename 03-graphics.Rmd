...
<!-- XXX due to time ... is this a book or just course notes? -->

We could have an entire course on creating graphics, but due to time we'll concentrate 
...
<!-- XXX Do you mean things like javascript here? is it worth a note -->
...
g_smooth = g + stat_smooth(colour = "steelblue", se = FALSE)
g_smooth
```
...
<!-- XXX potentially confusing since you haven't used method = anywhere, i would suggest either having method = smooth in your notes, or specifically saying in the stat_smooth function set the method argument to lm (it's default is smooth), if i was a noob i would be tempted to try stat_lm -->
...
<!-- XXX Either should be a note about things being removed in the output warning or use coord_cartesian -->
...
Around each point we draw a _kernel_ - figure \@ref(fig:3-11) (b). The kernel can be any symmetric function. 
For example, it could be a

<!-- XXX not strictly true, can be any non-negative function that integrates to 1. no requirement to be symmetric, it just makes sense. I think the integrates to one is important though, or area under the curve if you are trying to avoid being overly mathematical. could just say that kernel is a fancy word for probability distribution or similar -->
...
kernel should be. Intuitively, when we have many points, the kernel should be narrower. 

<!-- XXX A wide kernel has the effect of flattening out our histogram shape, similar to having fewer bins. A narrow kernel has the opposite effect. -->

## Boxplots


<!-- XXX you haven't actually mentioned continuous data in the prior graphics examples -->
...
<!-- XXX is it worht mentioning the -1? -->
...
g2 = count(bond, Actor) %>% 
  ...
  mutate(Actor=factor(Actor,
                      levels=c("Sean Connery", "George Lazenby", "Roger Moore", 
...
<!-- XXX i would argue this is reverse order of first appearance -->
